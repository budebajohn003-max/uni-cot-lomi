...
if not SECRET_KEY:
    # For build/initialization only - MUST be set in Render environment for production
    import uuid
    SECRET_KEY = f'build-time-key-{uuid.uuid4().hex[:32]}'
    print("⚠️  WARNING: DJANGO_SECRET_KEY not set. Using temporary build-time key.")
    print("   Set DJANGO_SECRET_KEY environment variable in Render for production!")
